  ...
  uint16_t log_now = 0;
        if (msg[0] == 0x17 && pps_write == 1) { // Send client data
          ...
          switch (msg_cycle) {
            ...
            case 7:
            {				
				uint8_t PPS_weekday = pps_values[PPS_DOW] - 1;  
				if (PPS_weekday == 0) {
					PPS_weekday = 7;
				}
				  
                tx_msg[1] = 0x69;
                tx_msg[6] = PPS_weekday + 0x10;     // high nibble: current heating program (0x10 = reduced, 0x00 = comfort), low nibble: day of week
                tx_msg[7] = 72;    // next heating program time (encoded as one increment translates to one 10 minute block)
...
*/
			  ...
			  if (msg[0] != 0x00) { // Info-Telegramme von der Therme (0x1D)
