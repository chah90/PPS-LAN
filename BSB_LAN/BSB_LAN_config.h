...
byte ip_addr[4] = {192,168,0,20};     // Please note the commas instead of dots!!!  Set useDHCP (above) to false if you want to use a fixed IP.
byte gateway_addr[4] = {192,168,0,1}; // Gateway address. This is usually your router's IP address. Please note the commas instead of dots!!! Ignored if first value is 0.
byte dns_addr[4] = {192,168,0,10};     // DNS server. Please note the commas instead of dots!!! Ignored if first value is 0.
...
// Setting bus type
uint8_t bus_type = 2;  // set bus system at boot: 0 = BSB, 1 = LPB, 2 = PPS
// BSB:
...
// parameter, destination (as in dest_address below, -1 means "default (dest_address) address")
    ...
    {15030, -1}, // Außentemperatur
    {15000, -1}, // Betriebsart
    {15001, -1}, // Präsenztaste
    {15008, -1}, // Raumtemperatur
    {15023, -1}, // TWW Nennsollwert
    {15021, -1}, // Trinkwassertemperatur
    {15040, -1}, // Uhrzeit
    {15003, -1}, // verbleibende Ferientage
    {15032, -1}, // Vorlauftemperatur
    {15041, -1}, // Wochentag
    {15035, -1}  // Zieltemperatur
...
char mqtt_broker_addr[33] = "192.168.0.11:1883";    // MQTT broker. Adjust LoggingMode to activate. Can be IP address or hostname. Optional port can be specified after trailing colon. If omitted, port defaults to 1883.
char MQTTUsername[65] = "mqtt";                 // Set username for MQTT broker here or set empty string if no username/password is used.
char MQTTPassword[65] = "mqtt4iobroker!";                 // Set password for MQTT broker here or set empty string if no password is used.
